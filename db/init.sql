...
BEGIN
  ...
  CREATE TABLE dbo.TIS_SPEDIZIONI (
    ...
    CONSTRAINT FK_SPED_CLIENTE  FOREIGN KEY (IdCliente) REFERENCES dbo.TIS_CLIENTI(Id),
    ...
    CONSTRAINT CK_SPED_PESO     CHECK (PesoKg IS NULL OR PesoKg >= 0),
    CONSTRAINT CK_SPED_DIM      CHECK (
        (LunghezzaCm IS NULL OR LunghezzaCm > 0) AND
        (LarghezzaCm IS NULL OR LarghezzaCm > 0) AND
        (AltezzaCm  IS NULL OR AltezzaCm  > 0)
    )
  );

  CREATE INDEX IX_SPED_IDORDINE  ON dbo.TIS_SPEDIZIONI(IdOrdine);
  CREATE INDEX IX_SPED_IDCLIENTE ON dbo.TIS_SPEDIZIONI(IdCliente);
...
BEGIN
  ...
  CREATE INDEX IX_MOV_MAGPROD ON dbo.TIS_MOVIMENTI(IdMagazzino, IdProdotto, DataMovimento DESC);
