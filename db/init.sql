...
IF NOT EXISTS (SELECT * FROM sys.objects)
WHERE object_id = OBJECT_ID(N'[dbo.][TF_STATO_ORDINE') AND type in (N'U'))


BEGIN


CREATE TABLE TF_STATO_ORDINE(Id INT primary key, Codice VARCHAR(50), Descrizione VARCHAR(60));

INSERT INTO TF_STATO_ORDINE VALUES (1, 'CREATO', 'Ordine Creato');
INSERT INTO TF_STATO_ORDINE VALUES (2, 'ANNULLATO', 'Ordine Annullato');
...
CREATE TABLE [dbo].[TIS_ORDINI](
	...
	CONSTRAINT STATO_ORDINE_FK FOREIGN KEY(IdStatoOrdine) REFERENCES TF_STATO_ORDINE(Id)
...
IF NOT EXISTS (SELECT 1 FROM sys.objects WHERE name='TF_STATO_CORRIERE' AND type='U')
BEGIN

CREATE TABLE [dbo].[TF_STATO_CORRIERE](
...
INSERT INTO TF_STATO_CORRIERE VALUES(1, 'LIBERO', 'Libero');

INSERT INTO TF_STATO_CORRIERE VALUES(2, 'INTRANSITO', 'In Transito');
...
BEGIN
  CREATE TABLE dbo.TIS_CORRIERI (
	...
	IdStatoCorriere INT,
	FK_STATO_CORRIERE FOREIGN KEY(IdStatoCorriere) REFERENCES dbo.TF_STATO_CORRIERE(Id)
  );
...
BEGIN
  ...
  CREATE INDEX IX_SPED_STATO     ON dbo.TIS_SPEDIZIONI(IdStatoSpedizione);
...
BEGIN
  CREATE TABLE dbo.TIS_SPEDIZIONI_STORICO(
	...
	CONSTRAINT FK_SPED_STORICO_STATO FOREIGN KEY (IdStato) REFERENCES dbo.TIS_SPEDIZIONE_STATO(Id)
  );
